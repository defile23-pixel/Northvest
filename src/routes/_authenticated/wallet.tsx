...
const CRYPTO_ASSETS = [
  { id: "BTC", label: "Bitcoin (BTC)", network: "Bitcoin", address: "bc1qe4n6yqtxa8s3h9txfl0uz7l595rqju9s3f5qla", min: 50 },
  // No real receiving address configured for these yet — deposits stay disabled for
  // them so nobody sends real funds to a placeholder and loses them. Withdrawals are
  // unaffected since the destination address there is the user's own, not ours.
  { id: "ETH", label: "Ethereum (ETH)", network: "ERC-20", address: null as string | null, min: 50 },
  { id: "USDT", label: "Tether (USDT)", network: "TRC-20", address: null as string | null, min: 20 },
] as const;
...
function WalletPage() {
  ...
  const [depositAsset, setDepositAsset] = useState<(typeof CRYPTO_ASSETS)[number]["id"]>("BTC");
  const [txHash, setTxHash] = useState("");
  const [withdrawMethod, setWithdrawMethod] = useState<"crypto" | "bank">("crypto");
  const [wAsset, setWAsset] = useState<(typeof CRYPTO_ASSETS)[number]["id"]>("USDT");
  ...
  const submitDeposit = async () => {
    ...
    setBusy("deposit");
    // Manually reviewed: an admin checks this hash on a block explorer and confirms
    // the amount/address match before approving — see set_transaction_status. The
    // transaction's own id doubles as a payment reference for support follow-up.
  ...
  const submitWithdraw = async () => {
    ...
    setBusy("withdraw");
    // Balance is intentionally NOT touched here. It's only debited once an admin
    // approves the request (server-side, re-checked against the live balance) —
    // see set_transaction_status. This avoids ever needing to "refund" a balance
    // that was deducted for a request that later gets rejected.
