...
function WalletPage() {
  ...
  // If an admin removes an asset key entirely (rather than just blanking the address),
  // fall back to whatever's actually still configured instead of pointing at nothing.
  useEffect(() => {
    if (CRYPTO_ASSETS.length === 0) return;
    if (!CRYPTO_ASSETS.some((a) => a.id === wAsset)) setWAsset(CRYPTO_ASSETS[0].id);
    if (!CRYPTO_ASSETS.some((a) => a.id === depositAsset)) setDepositAsset(CRYPTO_ASSETS.find((a) => a.address)?.id ?? CRYPTO_ASSETS[0].id);
  }, [assetSettings]);
