...
export const adminOverview = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const [users, newUsers, kycPending, txPending, revPending, wallets, completedTx] = await Promise.all([
      ...
      // Fixed: KYC submissions land in 'in_review', never 'pending' (that value isn't
      // even allowed by the table's CHECK constraint) — the old query always returned 0.
...
export const adminAdjustBalance = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    // Atomic: updates the wallet, writes a before/after audit_actions entry, and
    // notifies the user — all in one SECURITY DEFINER function. See migrations.
...
export const adminSetRole = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.admin) {
      ...
      // Refuses to remove the last remaining admin, so nobody can lock everyone out of /admin.
...
export const adminSetTxStatus = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await assertAdmin(context.supabase, context.userId);
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    // Runs as a single SECURITY DEFINER transaction: approving a deposit credits
    // cash_balance, approving a withdrawal debits it (re-checked server-side at this
    // moment), rejecting either one is a pure no-op on the wallet, and the decision
    // is recorded in admin_actions with a notification sent to the user.
...
export const adminGetKycDocumentUrl = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    await assertAdmin(context.supabase, context.userId);
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    // Service role bypasses the storage RLS that otherwise restricts each user's
    // folder to themselves, so this is the only way to read someone else's document.
